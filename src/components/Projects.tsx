    ...
    case "luis-felipe-pereira": // LFP - Architecture (Minimalist Grid)
      return (
        <div className="w-full h-full bg-stone-100 relative overflow-hidden">
          {/* Blueprint Grid */}
          <div
            className="absolute inset-0 bg-[linear-gradient(#00000010_1px,transparent_1px),linear-gradient(to_right,#00000010_1px,transparent_1px)]"
...
const ProjectCard: React.FC<{ project: Project; index: number }> = ({
  ...
  return (
    <motion.div
      ...
      className="group relative bg-white border-4 border-black p-0 perspective-1000"
    >
      {/* Dynamic Shadow */}
      <motion.div
      ...
      <div
        className="absolute -top-6 -left-6 w-16 h-16 bg-brutal-orange border-4 border-black flex items-center justify-center text-2xl font-black shadow-[4px_4px_0px_0px_#000] z-20"
      ...
      >
        ...
        <div className="absolute bottom-0 right-0 z-20 flex">
          <a
            ...
            className="bg-brutal-yellow border-t-4 border-l-4 border-black p-3 hover:bg-black hover:text-white transition-colors"
          >
      ...
      {/* Content */}
      <div
        className="p-6 md:p-8 bg-white relative"
      ...
      >
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {project.tech.map((tag) => (
            <span
              key={tag}
              className="px-2 py-1 text-xs font-bold font-mono border border-black bg-stone-100 text-stone-600 uppercase"
            >
              {tag}
            </span>
          ))}
      ...
      <div className="container mx-auto px-4">
        <div className="mb-16 flex flex-col md:flex-row items-start md:items-end justify-between gap-6">
          <div className="space-y-2">
            ...
            <h2 className="text-5xl md:text-7xl font-black uppercase tracking-tighter leading-none">
              <TextReveal text="TRABALHOS" />
              <br />
              <span className="text-brutal-orange">
                <TextReveal text="SELECIONADOS_" delay={0.2} />
              </span>
            </h2>
          </div>

          <p className="max-w-md text-lg font-medium border-l-4 border-black pl-4 py-2">
            Cases reais de transformação digital, de sites institucionais a
            plataformas globais.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-16">
          {projects.map((p, idx) => (
            <div
              key={p.id}
              className={idx % 2 !== 0 ? "md:translate-y-16" : ""}
            >
