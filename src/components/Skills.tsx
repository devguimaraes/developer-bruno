...
const Skills: React.FC<SkillsProps> = ({ className, ...props }) => {
  ...
  const getSkillDisplayData = (
    ...
    const textColor =
      skill.color === "bg-muted" || skill.color === "bg-primary"
        ? "text-white"
        : "text-black";
    ...
    >
      ...
      <div className="absolute inset-0 opacity-10 pointer-events-none flex flex-col justify-between rotate-3 scale-110">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="whitespace-nowrap text-[10rem] font-black leading-none text-black"
          >
              ...
              return (
                ...
                >
                  ...
                  <div className="absolute bottom-0 left-0 w-full h-2 bg-black flex gap-1 px-1">
                    {[1, 2, 3, 4, 5].map((i) => (
                      <div key={i} className="flex-1 bg-brutal-yellow"></div>
                    ))}
                  </div>
                  ...
                  <div className="p-4 h-full flex flex-col justify-between">
                    <div className="flex justify-between items-start">
                      ...
                      <Zap
                        size={16}
                        className="text-gray-300 group-hover:text-brutal-orange"
                      />
                    </div>
                    ...
                    {displayData.technologies.length > 0 && (
                      <div className="flex flex-wrap gap-1 justify-center mt-1">
                        {displayData.technologies
                          .slice(0, 2)
                          .map((tech, techIdx) => (
                            <span
                              key={techIdx}
                              className="text-[10px] font-mono bg-black text-white px-1 py-0.5 leading-none"
                            >
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
              <div className="text-center md:text-left">
                ...
                <div className="text-2xl font-black mt-1">
                  {new Set(displaySkills.map((s) => s.category)).size}{" "}
                  CATEGORIAS
                </div>
              </div>
              <div className="text-center md:text-left">
