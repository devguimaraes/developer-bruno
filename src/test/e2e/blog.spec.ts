...
test.describe('Fluxo do Blog', () => {
  ...
  test('Deve listar os posts do blog e permitir navegação para um post específico', async ({ page }) => {
    ...
    const firstPostTitle = await blogCards.first().locator('h3').textContent();
    
    // Clica no link "Ler mais" ou no card
    ...
    if (firstPostTitle) {
      await expect(page.locator('h1')).toContainText(firstPostTitle.trim());
    }
  ...
  test('Deve permitir voltar da página do post para a listagem', async ({ page }) => {
    ...
    const backButton = page.getByRole('link', { name: /Voltar para o blog/i }).or(page.getByRole('link', { name: /posts/i }));
