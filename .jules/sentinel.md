...
## 2025-05-22 - Incomplete Sanitization in Chart Component
**Vulnerability:** The previous fix for CSS injection in `ChartContainer` stripped `;`, `{`, and `}` but failed to remove `<` and `>`, allowing attackers to close the `<style>` tag and inject malicious scripts.
**Learning:** Regex-based sanitization for `dangerouslySetInnerHTML` must consider the surrounding HTML context. Stripping CSS syntax characters is insufficient if the attacker can break out of the HTML tag entirely.
**Prevention:** When injecting into `<style>` blocks, ensure that `<` and `>` are also sanitized or escaped, or avoid `dangerouslySetInnerHTML` for user-controlled styles if possible.
